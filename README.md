...
| Metric | PromQL |
|---|---|
| CPU usage per node | `100 - (avg by (instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)` |
...
| Network receive/transmit | `rate(node_network_receive_bytes_total[5m])` / `rate(node_network_transmit_bytes_total[5m])` |

![Node Exporter Dashboard](screenshots/monitoring/node-exporter-dashboard.png)

### Kubernetes object state (kube-state-metrics), scoped to `polling`

| Metric | PromQL |
|---|---|
| Pod restarts (CrashLoopBackOff) | `increase(kube_pod_container_status_restarts_total{namespace="polling"}[15m])` |
| Pods not Ready | `kube_pod_status_ready{namespace="polling", condition="false"}` |
| Pods stuck Pending | `kube_pod_status_phase{namespace="polling", phase="Pending"}` |
| OOMKilled containers | `kube_pod_container_status_last_terminated_reason{namespace="polling", reason="OOMKilled"}` |
| Deployment desired vs available | `kube_deployment_spec_replicas{namespace="polling"} - on(deployment) kube_deployment_status_replicas_available{namespace="polling"}` |
| PVC usage % | `kubelet_volume_stats_used_bytes{namespace="polling"} / kubelet_volume_stats_capacity_bytes{namespace="polling"} * 100` |
| HPA current vs desired replicas | `kube_horizontalpodautoscaler_status_current_replicas{namespace="polling"}` |

![kube-state-metrics Dashboard](screenshots/monitoring/kube-state-metrics-dashboard.png)
...
| | |
|---|---|
| Jenkins pipeline — all stages green | ![Jenkins Pipeline](screenshots/cicd/01-jenkins-pipeline-stages.png) |
| Jenkins — rollout verification log | ![Verify Rollout](screenshots/cicd/02-jenkins-verify-rollout-log.png) |
| SonarQube — quality gates passed | ![SonarQube](screenshots/cicd/03-sonarqube-quality-gates.png) |
| Amazon ECR — repositories | ![ECR Repos](screenshots/cicd/04-ecr-repositories.png) |
| Amazon ECR — versioned image tags | ![ECR Tags](screenshots/cicd/05-ecr-image-tags.png) |

### Kubernetes Cluster State (`namespace: polling`)

| | |
|---|---|
| Ingress | ![Ingress](screenshots/kubernetes/01-kubectl-get-ingress.png) |
| PVC (MySQL storage) | ![PVC](screenshots/kubernetes/02-kubectl-get-pvc.png) |
| NetworkPolicies | ![NetworkPolicy](screenshots/kubernetes/03-kubectl-get-networkpolicy.png) |
| StatefulSet (MySQL) | ![StatefulSet](screenshots/kubernetes/04-kubectl-get-statefulset.png) |
| HPA | ![HPA](screenshots/kubernetes/05-kubectl-get-hpa.png) |
| Secrets | ![Secrets](screenshots/kubernetes/06-kubectl-get-secrets.png) |
| ConfigMaps | ![ConfigMaps](screenshots/kubernetes/07-kubectl-get-configmaps.png) |

### Application Working

| | |
|---|---|
| Signup | ![Signup](screenshots/app/01-signup-page.png) |
| Registration success → login | ![Login Registered](screenshots/app/02-login-page-registered-success.png) |
| Login form | ![Login Filled](screenshots/app/03-login-page-filled.png) |
| Logged in — home page | ![App Home](screenshots/app/04-app-home-logged-in.png) |

### Monitoring

> Add your Grafana **Node Exporter** and **kube-state-metrics** dashboard screenshots to `screenshots/monitoring/` as `node-exporter-dashboard.png` and `kube-state-metrics-dashboard.png` — they're already referenced above in [Monitoring & Observability](#-monitoring--observability).
